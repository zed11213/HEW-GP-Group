...
// マウスの絶対値座標を取得する関数
D3DXVECTOR2 GetMouseAbsolute(void)
{
	//アクティブなウィンドウを取得
	HWND hWnd = GetActiveWindow();
	//マウス座標取得
	POINT cursorPos;
	GetCursorPos(&cursorPos);//マウス カーソルの位置を画面座標で取得
	ScreenToClient(hWnd, &cursorPos);//指定した画面座標をクライアント領域の座標に変換
	//マウス座標補正
	float cursorAbsPosX = SCREEN_WIDTH * (static_cast<float>(cursorPos.x) / GetClientWidth());
	float cursorAbsPosY = SCREEN_HEIGHT * (static_cast<float>(cursorPos.y) / GetClientHeight());

	D3DXVECTOR2 cursorAbsPos = D3DXVECTOR2(cursorAbsPosX, cursorAbsPosY);
	return cursorAbsPos;
}

// マウスの絶対値X座標を取得する関数(端950)
float GetMouseAbsoluteX(void) 
{
	D3DXVECTOR2 cursorAbsPos = GetMouseAbsolute();
	return cursorAbsPos.x;
}

// マウスの絶対値Y座標を取得する関数(端530)
float GetMouseAbsoluteY(void)
{
	D3DXVECTOR2 cursorAbsPos = GetMouseAbsolute();
	return cursorAbsPos.y;
}

// マウスの絶対値X座標(カメラ位置で補正)を取得する関数
float GetMouseAbsoluteXCamera(void)
{
	long cursorAbsPosX = GetMouseAbsoluteX();

	D3DXVECTOR2 camera = GetCameraMoveVolume();
	cursorAbsPosX += camera.x;

	return cursorAbsPosX;
}

// マウスの絶対値Y座標(カメラ位置で補正)を取得する関数
float GetMouseAbsoluteYCamera(void)
{
	long cursorAbsPosY = GetMouseAbsoluteY();

	D3DXVECTOR2 camera = GetCameraMoveVolume();
	cursorAbsPosY += camera.y;

	return cursorAbsPosY;
}
