...
static int g_soundIndexList[MAX_SOUND_NUM] = {NULL};
static int g_soundIndexListClearCou = 0;
...
void UpdateSound(void)
{
	...
	{
		g_soundIndexList[i] = -1;
	}

	//g_soundIndexListClearCou++;

	//if (g_soundIndexListClearCou >= 1)
	//{
	//	for (int i = 0; i < MAX_SOUND_NUM; i++)
	//	{
	//		g_soundIndexList[i] = -1;
	//	}
	//	g_soundIndexListClearCou = 0;
	//}
...
{
	//重複確認
	if(g_soundIndexList[MAX_SOUND_NUM - 1] != -1) return;
	for (int i = 0; i < MAX_SOUND_NUM; i++)
	{
		if (g_soundIndexList[i] == index) return;
	}
	...
	for (int i = 0; i < MAX_SOUND_NUM; i++)
	{
		if (g_soundIndexList[i] == -1) g_soundIndexList[i] = index;
		break;
	}
}

//音が重複しまくっても落ちないバージョン(重くりやすくなる)
//void PlaySound(int index, int loopCount)
//{
//	// 重複確認
//	if (g_soundIndexList[MAX_SOUND_NUM - 1] != -1) return;
...
//		do {
//			g_apSourceVoice[index]->GetState(&xa2state);
//		} while (xa2state.BuffersQueued > 0);
//	}
//
//	// オーディオバッファの登録
//	g_apSourceVoice[index]->SubmitSourceBuffer(&buffer);
